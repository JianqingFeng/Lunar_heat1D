...
#define NSKIN     20.0 // ratio of skin depth to first layer thickness: z_skin / dz[0]
#define NSKINBOT  50.0 //number of skin depths to bottom layer
#define NN        20   //scale factor for increasing layer thick. (low = fast inc.; 5 = nominal)
		  ...
		  int nrhs, const mxArray *prhs[] )
{
    ...
    plhs[1] = mxCreateDoubleMatrix(p->nlayers, 3, mxREAL);
    odepth = mxGetPr(plhs[1]);

    for (i=0; i<p->nlayers; i++) odepth[i] = p->layer[i].z;
    for (i=0; i<p->nlayers; i++) odepth[p->nlayers+i]= p->layer[i].dz;
    for (i=0; i<p->nlayers; i++) odepth[2*p->nlayers+i]= p->layer[i].rho;																		 
    /* Run thermal model */
...
//  t0d = t0s/sqrt(2.0) + HEATFLOW/kd + p->chi/30.0*t0s;
  ...
  t0s = pow((pow(cos(latitude),0.2752)-p->albedo)*S0/(EMIS*SIGMA*p->rau*p->rau),0.25) * pow(cos(latitude),0.25);
  t0d = t0s/sqrt(2.0);
